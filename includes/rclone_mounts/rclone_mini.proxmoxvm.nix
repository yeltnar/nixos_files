...
systemd.services.rclone-mini-mount = {
  ...
  serviceConfig = {
    ...
    ExecStart = "${pkgs.rclone}/bin/rclone mount --vfs-cache-mode full --allow-other --sftp-key-file ${private_key} --sftp-host ${host} --sftp-user ${user} --sftp-port ${port} :sftp:${remote_dir} ${local_dir}";
