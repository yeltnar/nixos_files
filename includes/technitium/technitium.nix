# man systemd-socket-proxyd
{
  ...
  ...
      ...
      name="technitium"; 
      user="drew"; 
      code_dir="/home/drew/playin/technitium"; 
      compose_file = pkgs.writeTextFile {name = "technitium_compose.yaml"; text = ''
      services:
        dns-server:
          ...
          ports:
            ...
            - "192.168.2.149:53:53/udp" #DNS service
            - "192.168.2.149:53:53/tcp" #DNS service
  ...
  systemd.services."${name}_compose_setup" = {
    path = with pkgs; [
      # git
    ];
    description = "${name}-compose_setup";
    ...
    unitConfig = {
      ConditionPathExists = "!${code_dir}/compose.yaml";
    };

    script = let 
      make_compose_script = pkgs.writeShellScript "technitium_make_compose_script" ''
        mkdir -p ${code_dir};
        cd ${code_dir};
        cat "${compose_file}" > ./compose.yaml
      '';
    in "${pkgs.util-linux}/bin/runuser -u ${user} -- ${pkgs.bash}/bin/bash '${make_compose_script}'";
    ...
    onSuccess = ["${name}_start.service"] ;
  };

  custom.compose.technitium = {
    allowedUDPPorts = [
      53
    ];
    allowedTCPPorts = [
    ...
    linger = true;
    use_run_env = false;
    test_string = "Press [CTRL + C] to stop...";
    backup_restore = true; 

    # super_user_clone = true;
    super_user_start = true;

    enable_clone_service = false;
